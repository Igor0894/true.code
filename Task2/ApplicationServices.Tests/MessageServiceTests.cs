...
namespace ApplicationServices.Tests
{
    internal class MessageServiceTests
    {
        ...
        private Guid _user2Guid = Guid.NewGuid();
        private Guid _user3Guid = Guid.NewGuid();
        private Guid _user5Guid = Guid.NewGuid();
        ...
        public void OnewTimeSetup()
        {
            ...
            var user2 = new User(_user2Guid, "user2");
            var user3 = new User(_user3Guid, "user3");
            var user4 = new User(new Guid(), "user4");
            var user5 = new User(_user5Guid, "user5");
            ...
            user2.Posts.Add(new UserPost(new Guid(), user2.Guid, _latestTime.AddMinutes(-10), "Latest post by user2"));
            user2.Posts.Add(new UserPost(new Guid(), user2.Guid, _latestTime.AddHours(-2), "Second post by user2"));
            user3.Posts.Add(new UserPost(new Guid(), user3.Guid, _latestTime.AddMinutes(-20), "Latest post by user3"));
            user3.Posts.Add(new UserPost(new Guid(), user3.Guid, _latestTime.AddHours(-2), "Second post by user3"));
            user4.Posts.Add(new UserPost(new Guid(), user4.Guid, _latestTime.AddHours(-1), "Latest post by user4"));
            user4.Posts.Add(new UserPost(new Guid(), user4.Guid, _latestTime.AddHours(-2), "Second post by user4"));
            user5.Posts.Add(new UserPost(new Guid(), user5.Guid, _latestTime.AddMinutes(-30), "Latest post by user5"));
        ...
        [Test]
        public async Task ShouldGetLastPosts()
        {
            IMessageFeeder messageFeeder = _messageFeederMock.Object;
            var messageService = new MessageService(messageFeeder);
            var postsResult = await messageService.GetLastPosts(4);
            postsResult.Should().HaveCount(4);
            postsResult[0].UserGuid.Should().Be(_user1Guid);
            postsResult[1].UserGuid.Should().Be(_user2Guid);
            postsResult[2].UserGuid.Should().Be(_user3Guid);
            postsResult[3].UserGuid.Should().Be(_user5Guid);
            postsResult[0].PublishTime.Should().Be(_latestTime);
            postsResult[1].PublishTime.Should().Be(_latestTime.AddMinutes(-10));
            postsResult[2].PublishTime.Should().Be(_latestTime.AddMinutes(-20));
            postsResult[3].PublishTime.Should().Be(_latestTime.AddMinutes(-30));
        }
    }
}
